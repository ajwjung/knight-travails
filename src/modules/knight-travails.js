const KnightTravails = (() => {
  ...
  const createAdjacencyList = () => {
    ...
    const graph = allCoordinates.map((coordinates) => {
      ...
      const filteredList = adjList.filter(
  ...
  const formatPath = (path) => {
    const pathMessage = `You made it in ${
      path.length - 1
  ...
  const getIndexOfCoordinates = (coord) => {
    ...
    for (let i = 0; i < 64; i += 1) {
      if (boardSquares[i][0] === coord[0] && boardSquares[i][1] === coord[1]) {
        return i;
      }
    }
  };
  ...
  const checkArrayPresent = (sourceArr, searchArr) => {
    for (const coordinates of sourceArr) {
      return coordinates[0] === searchArr[0] && coordinates[1] === searchArr[1];
    }
  };
  ...
  const getShortestPath = (start, end) => {
    ...
    while (queue.length > 0) {
      ...
      const adjacencyList = createAdjacencyList().find(
        (position) =>
